...
| # | Test case | Validates |
|---|---|---|
| RB-009 | Buy truncates down | 66 shares, never 67 |
| RB-010 | Sell truncates toward zero | 45 shares, not 46 |
| RB-011 | Residual bound | Untraded remainder is smaller than one share, under every policy |
| RB-012 | **FINDING** HALF_UP overdraft | BUY 67 IBM costs 10,050 against 9,900 raised; net cash -150 and the block is flagged unfundable |
| RB-013 | **FINDING** ROUND_UP overshoot | SELL 46 ORCL pushes it to 19.88%, crossing the target instead of approaching it |
| RB-014 | Sub-share gap | A 100 gap at 450/share produces no order; the same gap at 10/share sells 10 |

### TS-04 Input validation (`InputValidationTest`)

| # | Test case | Validates |
|---|---|---|
| RB-015 | Target sum | Targets not summing to 100 rejected, message quotes the actual sum |
| RB-016 | Bad price | 0, -1 and -150.50 all rejected before any division happens |
| RB-017 | Missing price | null rejected as a failed feed, never coerced to zero |

### TS-05 Cash, funding and sequencing (`CashFundingTest`)

| # | Test case | Validates |
|---|---|---|
| RB-018 | **FINDING** Truncation shortfall | Buy divides exactly, sell loses 1 to truncation; block is 1 short and unfundable |
| RB-019 | **FINDING** Sequencing | The same orders sent buys-first hit -9,900 running cash and cannot execute |
| RB-020 | Value conservation | Holdings plus cash equal 100,000 under all three rounding policies |

### TS-06 Business rules (`BusinessRuleTest`)

| # | Test case | Validates |
|---|---|---|
| RB-021 | Partial vesting | At 80% vested each target becomes 16%; BUY 40 IBM plus sells on all four other lines |
| RB-022 | Tolerance band | A 10 point band leaves a 10 point drift alone; 9.99 trades it |
| RB-023 | **FINDING** Zero variance unreachable | Residual is above zero yet inside 0.5 points, so the band is the real criterion |

### TS-07 Precision (`PrecisionTest`)

| # | Test case | Validates |
|---|---|---|
| RB-024 | **FINDING** Decimal vs double | 7000/0.07 gives 100,000 shares; the double result 99,999 is asserted as the wrong answer |
| RB-025 | Single rounding step | Rounding applied only at the share count, intermediates stay exact |

### TS-08 Metamorphic relations (`IdempotenceTest`)

| # | Test case | Validates |
|---|---|---|
| RB-026 | Idempotence | A second pass over a rebalanced account generates no churn |
...
| # | Test case | Validates |
|---|---|---|
| RB-027 | On-target lines never trade | No order wherever variance is zero |
| RB-028 | Direction opposes variance | Underweight lines only buy, overweight lines only sell |
| RB-029 | Variance never worsens | No position ends further from its target than it started |
| RB-030 | Residual under one share | Untraded remainder is always worth less than one share |
| RB-031 | Value conserved | Holdings plus cash always equal total assets |
| RB-032 | No short positions | No position is ever sold below zero |
| RB-033 | Deterministic output | The same account rebalanced twice gives identical orders |

### TS-11 Data driven (`DataDrivenTest`)

| # | Test case | Validates |
|---|---|---|
| RB-034 | Answer key | Every ABC line matches `account-abc.csv`, and the fixture inputs match the row so they cannot drift apart |
| RB-035 | Scenario table | Nine two-line scenarios from CSV covering rounding both ways, full exit and entry, penny prices, cents, and fractional percentages |
...
| # | Test case | Validates |
|---|---|---|
| RB-036 | Rounding policy sign-off | The intended policy is confirmed in writing by the business owner |
| RB-037 | Tolerance band sign-off | A real number is agreed; "zero" is challenged as unachievable |
