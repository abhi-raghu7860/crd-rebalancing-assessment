...
| `RoundingPolicyTest` | RB-009..014 | Each rounding policy's output and its cash consequence |
| `InputValidationTest` | RB-015..017 | Bad input rejected at the boundary with a named error |
| `CashFundingTest` | RB-018..020 | Whether the order block can actually be funded and executed |
| `BusinessRuleTest` | RB-021..023 | Vesting and the drift tolerance band |
| `PrecisionTest` | RB-024..025 | Decimal correctness and single-step rounding |
| `IdempotenceTest` | RB-026 | Metamorphic: a second pass generates no churn |
| `InvariantPropertyTest` | RB-027..033 | 7 invariants over 1,400 generated accounts |
| `DataDrivenTest` | RB-034..035 | CSV acceptance data |
...
| ID | Validates |
|---|---|
| RB-009 | Buy truncates down |
| RB-010 | Sell truncates toward zero |
| RB-011 | Untraded remainder is always less than one share, under all 3 policies |
| RB-012 | HALF_UP buys 67 IBM, net cash -150, block not fundable (FINDING) |
| RB-013 | ROUND_UP sells 46 ORCL, overshoots target to 19.88% (FINDING) |
| RB-014 | Gap worth less than one share generates no order |

### TS-04 Input validation (InputValidationTest)
| ID | Validates |
|---|---|
| RB-015 | target% not summing to 100 rejected |
| RB-016 | Zero or negative price rejected before any division |
| RB-017 | Null price rejected as a failed feed, not treated as zero |

### TS-05 Cash and funding (CashFundingTest)
| ID | Validates |
|---|---|
| RB-018 | Truncation does not guarantee fundability: 1 dollar short (FINDING) |
| RB-019 | Same orders buys-first reach -9,900 and cannot execute (FINDING) |
| RB-020 | Holdings plus cash still equal total assets, under all 3 policies |

### TS-06 Business rules (BusinessRuleTest)
| ID | Validates |
|---|---|
| RB-021 | 80% vested retargets every line: BUY 40 IBM, SELL 44/63/8/57 |
| RB-022 | Variance inside the tolerance band left alone |
| RB-023 | Zero variance unreachable, so the band is the real criterion (FINDING) |

### TS-07 Precision (PrecisionTest)
| ID | Validates |
|---|---|
| RB-024 | 7000/0.07 gives 100,000 shares; double gives 99,999 (FINDING) |
| RB-025 | Rounding applied once at the share step, not on intermediates |

### TS-08 Metamorphic (IdempotenceTest)
| ID | Validates |
|---|---|
| RB-026 | Second pass over a rebalanced account generates no churn |
...
| ID | Invariant |
|---|---|
| RB-027 | An on-target line is never traded |
| RB-028 | Order direction always opposes the variance sign |
| RB-029 | No position ends further from target than it started |
| RB-030 | Untraded remainder worth less than one share |
| RB-031 | Holdings plus cash equal total assets |
| RB-032 | No position sold below zero |
| RB-033 | Same input gives identical output |

### TS-11 Data driven (DataDrivenTest)
| ID | Validates |
|---|---|
| RB-034 | `account-abc.csv`: expected quantity per security, plus fixture matches the key |
| RB-035 | `two-line-scenarios.csv`: 9 scenarios covering rounding, exits, penny prices, fractions |

### TS-12 Manual only (not automated)
| ID | Activity |
|---|---|
| RB-036 | Get the rounding policy confirmed in writing by the business owner |
| RB-037 | Agree the tolerance band ("zero" is not achievable) |
